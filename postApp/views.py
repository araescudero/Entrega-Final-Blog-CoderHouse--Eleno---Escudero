...
def testing(request):
    ...
    if queryset:
        ...
        posts=Post.objects.filter(categoria__nombre = 'Testing')
        return render(request,'postApp/categoria_testing.html',{'posts':posts})
...
def Lenguajes(request):
    ...
    if queryset:
        ...
        posts=Post.objects.filter(categoria__nombre = 'Lenguajes')
        return render(request,'postApp/categoria_testing.html',{'posts':posts})
...
def machine_learning(request):
    ...
    if queryset:
        ...
        posts=Post.objects.filter(categoria__nombre = 'MachineLearning')
        return render(request,'postApp/categoria_testing.html',{'posts':posts})

def noticias_it(request):
    queryset = request.GET.get("buscar")
    if queryset:
        posts = Post.objects.filter(
            Q(titulo__icontains = queryset) |
        ...
        posts=Post.objects.filter(categoria__nombre = 'Noticias IT')
        return render(request,'postApp/categoria_testing.html',{'posts':posts})
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            subject = "Tienes una nueva consulta desde el blog CoderHouse" 
            body = {
            ...
            try:
                send_mail(subject, message, 'willyeleno44@gmail.com', ['willyeleno44@gmail.com']) 
